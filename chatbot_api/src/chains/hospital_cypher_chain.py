...
from utils.llm_config import llm_provider_kwargs
...
# Which physician has billed the least to STAR Assurances
MATCH (p:Payer)<-[c:COVERED_BY]-(v:Visit)-[t:TREATS]-(phy:Physician)
WHERE p.name = 'STAR Assurances'
RETURN phy.name AS physician_name, SUM(c.billing_amount) AS total_billed
ORDER BY total_billed
LIMIT 1

# Which governorate had the largest percent increase in STAR Assurances
# visits from 2022 to 2023?
MATCH (h:Hospital)<-[:AT]-(v:Visit)-[:COVERED_BY]->(p:Payer)
WHERE p.name = 'STAR Assurances' AND v.admission_date >= '2022-01-01' AND
v.admission_date < '2024-01-01'
WITH h.governorate AS governorate, COUNT(v) AS visit_count,
     SUM(CASE WHEN v.admission_date >= '2022-01-01' AND
     v.admission_date < '2023-01-01' THEN 1 ELSE 0 END) AS count_2022,
     SUM(CASE WHEN v.admission_date >= '2023-01-01' AND
     v.admission_date < '2024-01-01' THEN 1 ELSE 0 END) AS count_2023
WITH governorate, visit_count, count_2022, count_2023,
     (toFloat(count_2023) - toFloat(count_2022)) / toFloat(count_2022) * 100
     AS percent_increase
RETURN governorate, percent_increase
ORDER BY percent_increase DESC
LIMIT 1

# How many non-emergency patients in Sfax have written reviews?
MATCH (r:Review)<-[:WRITES]-(v:Visit)-[:AT]->(h:Hospital)
WHERE h.governorate = 'Sfax' and v.admission_type <> 'Emergency'
RETURN count(*)
...
Payer names are one of: 'CNAM', 'STAR Assurances', 'Maghrebia Assurances',
'GAT Assurances', 'Assurances BIAT'

A visit is considered open if its status is 'OPEN' and the discharge date is
missing.
Hospital governorates are the Tunisian governorate names in full
(e.g. "Tunis", "Sfax", "Sousse", "Monastir", "Nabeul"). Do not abbreviate
them.
...
hospital_cypher_chain = GraphCypherQAChain.from_llm(
    cypher_llm=ChatOpenAI(
        model=HOSPITAL_CYPHER_MODEL, temperature=0, **llm_provider_kwargs()
    ),
    qa_llm=ChatOpenAI(model=HOSPITAL_QA_MODEL, temperature=0, **llm_provider_kwargs()),
    graph=graph,
    verbose=True,
