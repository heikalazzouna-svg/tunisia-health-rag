...
with st.sidebar:
    ...
    st.markdown("- What is the current wait time at Hopital Charles Nicolle?")
    st.markdown(
    ...
    st.markdown("- What is the average billing amount for CNAM visits?")
    ...
    st.markdown(
        "- Which governorate had the largest percent increase in CNAM visits "
        "from 2022 to 2023?"
    )
    st.markdown(
        "- What is the average billing amount per day for STAR Assurances patients?"
    )
    st.markdown("- How many reviews have been written from patients in Sfax?")
    st.markdown(
    ...
    st.markdown("- What is the ID for physician Aymen Chaabane?")
    st.markdown(
